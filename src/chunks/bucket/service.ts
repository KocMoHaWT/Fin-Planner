...
export class BucketService implements IBucketService {
    ...
    async getLogs(req: CustomRequest, res: Response): Promise<Response> {
        const logs = await this.repository.getLogsByBucketId(+req.params.id, req.user.id);
        return res.status(200).json(logs);
    }

    async moneyTransfer(req: CustomRequest, res: Response): Promise<Response> {
        await this.repository.createActivityLog(req.body.bucketId, req.body.incomeId, req.body.ammount, req.body.direction);
